...
# Package imports for the shared app; local imports keep standalone use working.
if __package__:
...
def measurement_panel():
    try:
        ...
        return
    uploaded = st.file_uploader('Upload a portrait phone image', type=['jpg', 'jpeg', 'png'], key='module2.upload')
    distance_m = st.number_input('Camera-to-book distance (m)', min_value=0.001, value=2.134,
                                 step=0.001, format='%.3f', key='module2.distance',
    ...
    try:
        ...
        st.error(str(exc))
        return
    if st.button('Reset Corners', key='module2.reset_corners'):
        state = new_selection(token, state['generation'] + 1)
        st.session_state[STATE_KEY] = state
    ...
    with image_column:
        ...
        event = streamlit_image_coordinates(
            preview, key=f'module2.corners_{token[1]}_{state["generation"]}',
    ...
    with corner_column:
        ...
        if st.button('Calculate', type='primary', disabled=count != 4, key='module2.calculate'):
            try:
    ...
    if state['result'] is not None:
        ...
        image_download.download_button('Download annotated image', state['annotation_png'],
                                       file_name=f'{stem}_annotated.png', mime='image/png', key='module2.download_image')
        json_download.download_button('Download JSON result', json.dumps(result, indent=2, allow_nan=False),
                                      file_name=f'{stem}_measurement.json', mime='application/json', key='module2.download_json')
